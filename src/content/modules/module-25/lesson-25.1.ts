...
import theory25_1 from './lesson-25.1.md?raw';

export const lesson25_1: Lesson = {
  "id": "25.1",
  "moduleId": 25,
  "title": "Wiersz poleceń Linuksa i bash dla testerów",
  "description": "Terminal, grep, find, curl, jq, procesy, pliki, kody wyjścia, potoki i skrypty diagnostyczne używane w testach i CI.",
  "order": 1,
  ...
  "content": {
    "objective": "Po ukończeniu lekcji potrafisz używać terminala jako narzędzia diagnostycznego: analizować logi, wykonywać requesty, filtrować JSON, sprawdzać procesy i pisać proste skrypty wspierające testy oraz CI.",
    "theory": theory25_1,
    "codeExamples": [
      "#!/usr/bin/env bash\nset -euo pipefail\n\n: \"${BASE_URL:?BASE_URL is required}\"\n\ncurl -fsS \"$BASE_URL/health\" | jq '.status'\nnode --version\nnpx playwright --version\n",
      "# Analiza logów testowego przebiegu\ngrep \"$CORRELATION_ID\" logs/app.log   | jq -R 'select(length > 0)'   | tail -50\n"
    ],
    "exercises": [
      {
        "id": "ex-25-1-1",
        "title": "Procedura diagnostyczna",
        "description": "Dla tematu „Wiersz poleceń Linuksa i bash dla testerów” zapisz procedurę: komendy, oczekiwane wyniki i możliwe wnioski."
      },
      {
        "id": "ex-25-1-2",
    ...
    "quiz": [
      ...
      {
        "id": "q25-1-8",
        "question": "Najważniejsza idea lekcji „Wiersz poleceń Linuksa i bash dla testerów” to:",
        "options": [
