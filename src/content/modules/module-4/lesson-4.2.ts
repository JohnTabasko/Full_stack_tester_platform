...
import theory4_2 from './lesson-4.2.md?raw';

export const lesson4_2: Lesson = {
  "id": "4.2",
  "moduleId": 4,
  "title": "Ramki iframe i ramki zagnieżdżone",
  "description": "Praca z iframe, frameLocator, ramki zagnieżdżone, cross-origin i stabilne interakcje w ramkach.",
  "order": 2,
  ...
  "content": {
    "objective": "Po ukończeniu lekcji potrafisz stosować technikę: Ramki iframe i ramki zagnieżdżone, rozumiesz jej ryzyka i umiesz projektować stabilne testy z diagnostyką oraz izolacją stanu.",
    "theory": theory4_2,
    "codeExamples": [
      "const frame = page.frameLocator('iframe[name=\"payment\"]');\nawait frame.getByLabel('Numer karty').fill('4242424242424242');\nawait frame.getByRole('button', { name: 'Zapłać' }).click();\n",
      "const nested = page\n  .frameLocator('#outer-frame')\n  .frameLocator('#inner-frame');\n\nawait expect(nested.getByText('Gotowe')).toBeVisible();\n"
    ],
    "exercises": [
      {
        "id": "ex-4-2-1",
        "title": "Scenariusz podstawowy",
        "description": "Napisz test wykorzystujący technikę „Ramki iframe i ramki zagnieżdżone” i sprawdzający widoczny rezultat."
      },
      {
        "id": "ex-4-2-2",
    ...
    "quiz": [
      {
        "id": "q4-2-1",
        "question": "Co jest najważniejsze przy technice „Ramki iframe i ramki zagnieżdżone”?",
        "options": [
