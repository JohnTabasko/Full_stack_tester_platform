...
import theory14_5 from './lesson-14.5.md?raw';

export const lesson14_5: Lesson = {
  "id": "14.5",
  "moduleId": 14,
  "title": "Testowanie zgodności i wymagań regulacyjnych",
  "description": "RODO/GDPR, zgoda cookies, prawo do usunięcia danych, WCAG, ślad audytowy, PII, PCI DSS i wymuszanie HTTPS.",
  "order": 5,
  "difficulty": "advanced",
  "tags": [
    "security",
  ...
  "content": {
    "objective": "Po ukończeniu lekcji potrafisz projektować testy dla obszaru „Testowanie zgodności i wymagań regulacyjnych”, łącząc automatyzację z analizą ryzyka, dowodami audytowymi i świadomą interpretacją wyników.",
    "theory": theory14_5,
    "codeExamples": [
      "// Cookie consent — koncepcyjnie\nawait page.goto('/');\nawait expectNoMarketingCookies(context);\nawait page.getByRole('button', { name: 'Odrzuć' }).click();\nawait expectOnlyEssentialCookies(context);\n",
      "// Right to erasure — koncepcyjnie\nawait privacyApi.requestAccountDeletion(user.id);\nawait expect.poll(() => usersApi.get(user.id).then(r => r.status())).toBe(404);\nawait expectAuditEvent('account.deleted', user.id);\n"
    ],
    "exercises": [
      {
        "id": "ex-14-5-1",
        "title": "Mapa ryzyk",
        "description": "Dla tematu „Testowanie zgodności i wymagań regulacyjnych” wypisz ryzyka, dane testowe i oczekiwane dowody."
      },
      {
        "id": "ex-14-5-2",
    ...
    "quiz": [
      ...
      {
        "id": "q14-5-8",
        "question": "Najważniejsza zasada lekcji „Testowanie zgodności i wymagań regulacyjnych” to:",
        "options": [
