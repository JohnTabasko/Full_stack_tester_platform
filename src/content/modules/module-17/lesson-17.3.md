...
Ta lekcja koncentruje się na: **klasy równoważności, wartości brzegowe, tablice decyzyjne, przejścia stanów, pairwise testing i projektowanie danych testowych**. Główne ryzyko: **zespół tworzy przypadki testowe intuicyjnie, przez co ma dużo powtórzeń, ale pomija wartości brzegowe i kombinacje wysokiego ryzyka**. Po lekturze powinieneś umieć używać wiedzy testerskiej do projektowania lepszych testów manualnych, eksploracyjnych i automatycznych.

## Sytuacja przewodnia

formularz pożyczki zależy od wieku, dochodu, kraju, historii klienta i kwoty, więc pełna kombinatoryka jest zbyt droga

## 1. Po co techniki projektowania

Techniki pomagają wybrać mało testów o dużej wartości. Bez techniki łatwo pisać wiele podobnych przypadków i nadal pomijać ryzyko.

## 2. Klasy równoważności

Klasy równoważności grupują dane, które powinny być traktowane tak samo. Testujesz reprezentantów klas, zamiast każdej wartości.

## 3. Wartości brzegowe

Błędy często pojawiają się na granicach: minimum, maksimum, pierwszy dzień, ostatni dzień, limit długości, przejście statusu.

## 4. Tablice decyzyjne

Tablice decyzyjne pomagają przy regułach zależnych od wielu warunków. Pokazują kombinacje i oczekiwane decyzje.

## 5. Przejścia stanów i pairwise

Przejścia stanów są dobre dla workflow, a pairwise ogranicza liczbę kombinacji przy wielu parametrach.

## Przykład referencyjny

```markdown
# Przykład: klasy równoważności dla wieku

Pole: wiek klienta

- niepoprawne: < 18
- poprawne: 18–65
- wymagające dodatkowej weryfikacji: > 65
