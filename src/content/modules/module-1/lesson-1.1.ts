...
export const lesson1_1: Lesson = {
  ...
  "description": "Architektura Playwrighta, porównanie z Selenium i Cypress, hierarchia Browser → Context → Page oraz dlaczego Playwright minimalizuje koszty utrzymania testów w projektach komercyjnych.",
  "order": 1,
  "difficulty": "beginner",
  "tags": [
    "playwright",
  ...
  "content": {
    "objective": "Po ukończeniu lekcji rozumiesz, czym jest Playwright, jakie problemy rozwiązuje, jak różni się od starszych narzędzi automatyzacji i dlaczego jego architektura sprzyja stabilnym testom full stack.",
    "theory": theory1_1,
    "codeExamples": [
      "import { test, expect } from '@playwright/test';\n\ntest('użytkownik widzi stronę główną aplikacji', async ({ page }) => {\n  await page.goto('/');\n  await expect(page.getByRole('main')).toBeVisible();\n});\n",
      "// Izolowane konteksty pozwalają testować dwóch użytkowników.\nconst adminContext = await browser.newContext({ storageState: 'admin.json' });\nconst userContext = await browser.newContext({ storageState: 'user.json' });\n\nconst adminPage = await adminContext.newPage();\nconst userPage = await userContext.newPage();\n"
    ],
    "exercises": [
      {
        "id": "ex-1-1-1",
        "title": "Notatka koncepcyjna",
        "description": "Wyjaśnij własnymi słowami temat „Czym jest Playwright?” i wskaż, jakie ryzyko rozwiązuje w projekcie testowym."
      },
      {
        "id": "ex-1-1-2",
    ...
    "quiz": [
      {
        "id": "q1-1-1",
        "question": "Jaki jest najważniejszy cel lekcji „Czym jest Playwright?”?",
        "options": [
