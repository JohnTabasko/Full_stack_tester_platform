...
export const lesson5_1: Lesson = {
  ...
  "content": {
    "objective": "Po ukończeniu lekcji rozumiesz runner testów Playwright jako mechanizm organizujący wykonanie testów, potrafisz używać describe, hooków, test.step, adnotacji i testInfo w sposób zwiększający czytelność oraz diagnostykę.",
    "theory": theory5_1,
    "codeExamples": [
      "import { test, expect } from '@playwright/test';\n\ntest.describe('Checkout', () => {\n  test('gość może złożyć zamówienie @smoke', async ({ page }, testInfo) => {\n    testInfo.annotations.push({ type: 'risk', description: 'critical revenue flow' });\n\n    await test.step('Otwarcie koszyka', async () => {\n      await page.goto('/cart');\n    });\n\n    await test.step('Złożenie zamówienia', async () => {\n      await page.getByRole('button', { name: 'Złóż zamówienie' }).click();\n    });\n\n    await test.step('Potwierdzenie sukcesu', async () => {\n      await expect(page.getByText('Dziękujemy za zakup')).toBeVisible();\n    });\n  });\n});\n",
    ...
    "exercises": [
      {
        ...
        "description": "Dla tematu „Runner testów Playwright” zaprojektuj strukturę test.describe/testów lub fixture odpowiadającą realnemu modułowi aplikacji."
      },
      {
        "id": "ex-5-1-2",
    ...
    "quiz": [
      ...
      {
        "id": "q5-1-8",
        "question": "Najważniejsza zasada lekcji „Runner testów Playwright” to:",
        "options": [
